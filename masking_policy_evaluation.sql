...
select min("DATE"), max("DATE") from HAVEN_RAW.BIGQUERY.TARGET_01_SEARCHES_AND_BOOKINGS_SUMMARY;

select count(*) from HAVEN_RAW.BIGQUERY.TARGET_01_SEARCHES_AND_BOOKINGS_SUMMARY;
select count(*) from "HAVEN_STORE"."COMMERCIAL"."SEARCHES_AND_BOOKINGS_SUMMARY";
...
show roles like '_%__reader';

--KNOEMA_ECONOMY_DATA_ATLAS__ECONOMY__READER
show grants of role haven_work__test_master__reader;

show grants of role KNOEMA_ECONOMY_DATA_ATLAS__ECONOMY__READER;

use role accountadmin;
drop role haven_work__test_master__reader;
drop role KNOEMA_ECONOMY_DATA_ATLAS__ECONOMY__READER;
...
select ship_name from butlins_raw.seaware.ship
union
select ship_name from haven_raw.seaware.ship
union
select ship_name from warner_raw.seaware.ship;


use database haven_base;
show schemas like 'seaware%';


select table_schema, table_name, column_name, ordinal_position, data_type, character_maximum_length
from information_schema.columns
where table_catalog = 'HAVEN_BASE' and table_schema = 'SEAWARE' and table_name = 'CC_AUS_FILES'
limit 100;


select COALESCE(AVG(row_length), 0) from
(SELECT length(web_session_info) AS row_length FROM haven_base.seaware.client_web_session WHERE web_session_info is not null
 order by length(web_session_info) desc
 limit 5000);
...
use role dba;
use schema haven_store.commercial;

create view haven_store.commercial.booking_ids_by_channel
as select "DATE", channel, channel_level_2, channel_level_3, hol_booking_id, revenue
from "HAVEN_RAW"."BIGQUERY"."03_BOOKING_IDS_BY_CHANNEL";

select * from haven_store.commercial.booking_ids_by_channel;
...
revoke select on view haven_base.seaware.client from role haven_commercial_test;
grant role _haven_base__seaware__reader to role haven_commercial_test;

use role dba;

select "DATE", count(*) from "HAVEN_RAW"."BIGQUERY"."03_BOOKING_IDS_BY_CHANNEL"
group by 1 order by 1 desc;


select * from haven_base.information_schema.columns where contains(table_name, 'CLIENT');
