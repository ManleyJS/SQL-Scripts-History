...
def run(session, databaseName, schemaName):
  ...
  semDBList      = ['HAVEN_SEMANTIC', 'HAVEN_MASTER', 'HAVEN_DATA_SCIENCE']
  customEnvDBSuf = ['_AA_DEV', 'RMS_DEV']

  masterDB       = 'HAVEN_MASTER'
  dataSciDBLis   = ['HAVEN_DATA_SCIENCE']

  cicdRoles      = ['haven_dbt_transform', 'haven_dbt_transform_finance', 'haven_dbt_transform_pipelines',
                    'haven_dbt_transform_pitch_perfect' ]
  ...
  pipelineRole   = 'haven_dbt_pipeline'

  AADevDBTRole   = 'haven_dbt_transform_align_alytics_dev'
  RMDevDBTRole   = 'haven_dbt_transform_rms_core_dev'
  ...
  def setConsumptionLayerAccess(dbName, schemaName):
    ...
    if dbName == "HAVEN_PIPELINE":
      sqlText = "grant role _{0}__{1}__creator to role {2}".format(dbName, schemaName, pipelineRole)
      result = session.sql(sqlText).collect()
  ...
  def setDevelopmentEnvironmentAccess(dbName, schemaName):
    ...
    if dbName in semDBList:
        sqlText = "grant role _{0}__{1}__reader to role {2}".format(dbName, schemaName, engrRoleDev)
    else:
...
## main body of code
  ...
  ## rework the code so that we handle _AA_DEV and _RMS_DEV database schemaa differently
  ## should ignore anything that ends with these suffixes for now

  if db in consDBList or db in semDBList:
