...
--create schema training.deleteme;
--show roles like '_training__deleteme__%';
drop schema haven_raw.kappture;
drop role _haven_raw__kappture__CREATOR;
drop role _haven_raw__kappture__OWNER;
drop role _haven_raw__kappture__READER;
drop role _haven_raw__kappture__USAGE;
drop role _haven_raw__kappture__WRITER;

drop procedure schema_role_creation(string , string );

CREATE OR REPLACE PROCEDURE schema_role_creation(databaseName STRING, schemaName STRING)
COPY GRANTS
  RETURNS STRING
  LANGUAGE PYTHON
...
use role haven_schema_setup;
call schema_role_creation('haven_base', 'kappture');
