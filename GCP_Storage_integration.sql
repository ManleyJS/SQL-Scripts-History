...
GRANT ROLE _haven_development__schema TO ROLE dba;

use role dba;
use database haven_development;

SHOW SCHEMAs;

--CREATE SCHEMA haven_development.staging_data;

USE SCHEMA haven_development.staging_data;

create or replace file format haven_development.staging_data.gcs_snowflake_storage_integration_format
  type = csv
  compression = gzip
  field_delimiter = ','
...
SHOW FILE FORMATS;
SHOW STAGES LIKE 'gcs%';

USE ROLE dba;

CREATE OR REPLACE stage haven_development.staging_data.gcs_snowflake_storage_integration_stage
  url = 'gcs://snowflake-1274/'
  storage_integration = gcs_snowflake_storage_integration
  file_format = haven_development.staging_data.gcs_snowflake_storage_integration_format
;

ls @gcs_snowflake_storage_integration_stage;
...
ls @haven_development.staging_data.gcs_snowflake_storage_integration_stage;

COPY INTO @haven_development.staging_data.gcs_snowflake_storage_integration_stage/FACEBOOK_DATA.CDV.GZ
FROM (SELECT * FROM HAVEN_DEVELOPMENT.FACEBOOK.HOLS_TOPLINE LIMIT 100);

RM @haven_development.staging_data.gcs_snowflake_storage_integration_stage/FACEBOOK_DATA.CDV.GZ;