...
drop procedure haven_master.procedures.changer_object_ownership(string, string, string);

CREATE OR REPLACE PROCEDURE haven_master.procedures.change_object_ownership(
        databaseName STRING, schemaName STRING, newOwner STRING)
COPY GRANTS
  RETURNS STRING
  LANGUAGE PYTHON
...
AS
$$
def run(session, dbName, scName, newOwner):
  ...
  def grantOwnershipAndDefaultPermissions(dbName, scName, newOwner, object):
    ...
    if len(error) == 0:
      sqlText = "grant select on {3} {0}.{1}.{2} to role _{0}__{1}__reader".format(
        dbName, scName, object[0], objType)
      try:
        result = session.sql(sqlText).collect()
      except Exception as e:
        error = e.message
    ...
    if len(error) == 0 and objType == 'TABLE':
      sqlText = "grant insert, update, delete, truncate on {0}.{1}.{2} to role _{0}__{1}__writer".format(
...
call haven_master.procedures.change_object_ownership('haven_store_dev', 'TEST_OWNER_PERMISSIONS', 'haven_dbt_transform_finance_dev');
